...
def main():
    ...
    if save_out_video:
        ...
        videoFullWriter = cv2.VideoWriter(
                ...
                for m in m_queue:
                    ...
                    if mk == 0 and mv >= 20:
                        ...
                        videoWriter = cv2.VideoWriter(
                                        os.path.join(args.out_video_root, f'vis_{idx_video}_{os.path.basename(args.video_path)}.mp4'), 
                                        fourcc, fps, size
                                    )
                        processing_queue = [return_frame]
                    if mk == 2 and mv >= 2 and write_video:
                        action_bool[mk] = True
                    if mk == 4 and mv >= 2 and write_video:
                        action_bool[mk] = True
                    if mk == 5 and mv >= 1 and write_video:
                        action_bool[mk] = True
                    if mk == 6 and mv >= 2 and write_video:
                        action_bool[mk] = True
                    if mk != 0 and mk != 7 and mk != 6 and mk != 5 and mk != 2 and mv >= 1 and write_video:
                        action_bool[mk] = True
                    if mk == 7 and mv >= 5 and write_video:
                        action_bool[mk] = True
                        write_video = False
    ...
    #         img_draw_path = os.path.join(folder_8poses, img_draw_name)
    #         mmcv.imwrite(img_draw, img_draw_path)
    # print ("time_matching: {}".format(time.time() - time_matching))
    # print ("final time: {}".format(time.time() - start_time))
