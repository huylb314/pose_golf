...
pose_limb_color = palette[[16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16]]
pose_kpt_color = palette[[16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16]]

FREQ_START_SPLIT = 20
FREQ_END_SPLIT = 3
threshold = 10
fps = 10
width, height = int(360), int(640)
size_hstack = (width*5, height)
...
folder_parent = "/home/ddes-0351/Documents/source_code/react_native/rtmp_py_server/"
...
def main():
    ...
    if save_out_video:
        ...
        videoFullPath = os.path.join(args.out_video_root, f'vis_{current_timestamp}.mp4')
        print (videoFullPath)
        # videoFullWriter = cv2.VideoWriter(videoFullPath, fourcc, fps, size)
    ...
    for msg in img_sub.listen():
        ...
        if mess != 1:
            ...
            img = cv2.imread(os.path.join(folder_parent, img_path))
            ...
            if len(bboxes) > 0:
                ...
                if len(m_queue) > 0:
                    print (m_queue)
                    return_frame = keypoint_pair_angles(keypoints, dict_angles)
                    angles, angles_draw, angles_coor, angles_name, kp_coco_angles = return_frame
                    ...
                    for m in m_queue:
                        ...
                        if mk == 0 and mv >= 15:
                            ...
                            vis_img = show_result_original(img,
                                            ...
                                            show=False)
                            image_queue = [vis_img]
                        elif mk == 7 and mv >= 5 and write_video:
                            action_bool[mk] = True
                            write_video = False
